...
- **「插件目录」面板**：卡片化分组展示，带来源徽标、搜索框、**已安装版本号**与功能描述；若插件 package.json 提供 GitHub 仓库地址，卡片会显示 **GitHub 按钮**，可直接打开详细功能介绍（v0.8）；
- **自装插件更新（v0.6）**：「插件目录」面板顶部带「更新」区块——打开面板自动检查 npm registry，统计可更新总数（可一键全部更新），每个 registry 插件卡片带独立「更新」按钮；Desktop 托管的 `link:` 插件显示「随桌面版更新」，本地链接显示「本地插件」，避免把 pnpm 退出码 0 误报成实际更新成功；
